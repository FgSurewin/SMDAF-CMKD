...
from models.loss.kd_loss import KDLoss
...
class ImageAudioKDCompositeBaseModel(L.LightningModule):
    def __init__(self, num_classes, label_names, kd_config, **kwargs):
        super().__init__()
        self.num_classes = num_classes
        self.label_names = label_names
        self.kd_config = kd_config
        # self.kd_loss = KDLoss(**kd_config)
        ...
        print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
        print(f"KD Config: {self.kd_config}")
        print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")

    def parse_batch(self, batch):
        """Override this method in your model to parse the batch correctly."""
        raise NotImplementedError

    def training_step(self, batch, batch_idx):
        data = self.parse_batch(batch)
        logits_dict = self(data)
        kd_loss = KDLoss(**self.kd_config)
        loss = kd_loss(logits_dict)
        ...
        metrics_dict = self.train_metrics(logits_dict["student_logits"], data["labels"])
        self.log_dict(
            metrics_dict, on_step=False, on_epoch=True, prog_bar=True, logger=True
        )
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        data = self.parse_batch(batch)
        logits_dict = self(data)
        loss = F.cross_entropy(logits_dict["student_logits"], data["labels"])
        self.log(
            "val_loss", loss, on_step=False, on_epoch=True, prog_bar=True, logger=True
        )
        val_metrics_dict = self.val_metrics(
            logits_dict["student_logits"], data["labels"]
        )
        self.log_dict(
            val_metrics_dict, on_step=False, on_epoch=True, prog_bar=True, logger=True
        )
        return loss
    ...
    def test_step(self, batch, batch_idx):
        data = self.parse_batch(batch)
        logits_dict = self(data)
        self.test_pred_list += logits_dict["student_logits"].argmax(dim=1).tolist()
        self.test_target_list += data["labels"].tolist()
        self.test_logits_list.append(logits_dict["student_logits"].tolist())
        loss = F.cross_entropy(logits_dict["student_logits"], data["labels"])
        self.test_metrics.update(logits_dict["student_logits"], data["labels"])
        self.conf_matrix.update(logits_dict["student_logits"], data["labels"])
        self.log(
